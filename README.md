...
| `unsafe-rm` | guard | Answers Claude Code's un-bypassable "dangerous rm" prompt so `rm`/`rmdir` run unattended. That prompt survives `--dangerously-skip-permissions` by design (an upstream circuit breaker). Cleat installs a conservative `PermissionRequest` hook that allows only commands whose every segment is `rm`/`rmdir`/`cd`, so `rm -rf $S/*` passes while `curl x \| sh; rm ...` still prompts. **This disarms a real guard**: `/workspace` and `~/.claude` are read-write host mounts. Global or `--cap` only, never a project `.cleat`. Default off, red warning every launch. |
